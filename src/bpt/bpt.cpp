...
inline index_t* find(internal_node_t& node, const key_t& key) {
    ...
    // because the end of the index range is an empty string, so
    //if we search the empty key(when merge internal nodes), we
    //need to return the second last one
    if (node.n > 1) {
...
int bplus_tree::search_range(key_t* left, const key_t& right, value_t* values, size_t max, bool* next) const
{
    if (NULL == left || keycmp(*left, right) > 0)
    {
        return -1;
    }
...
void bplus_tree::remove_from_index(off_t offset, internal_node_t& node, const key_t& key)
{
...
bool bplus_tree::borrow_key(bool from_right, internal_node_t& borrower, off_t offset)
{
    ...
    {
        child_t where_to_lend;
        child_t where_to_put;
        ...
        else
        {
            ...
            // We shouldn't change where_to_put->key, because it just records the largest 
            //info but we only changes a new one which have been the smallest one
...
void bplus_tree::change_parent_child(off_t parent, const key_t&o, const key_t& n)
{
...
void bplus_tree::merge_keys(index_t* where, internal_node_t& node, internal_node_t& next, bool change_where_key)
{
    if (change_where_key)
    {
...
void bplus_tree::insert_record_no_split(leaf_node_t* leaf, const key_t& key, const value_t& value)
{
